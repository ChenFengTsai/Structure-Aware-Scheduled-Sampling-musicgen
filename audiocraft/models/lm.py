...
# ============================================================
# CHANGES FROM ORIGINAL lm.py
# ============================================================
# 1. Added `_compute_per_step_ce()` — shared helper that returns
#    per-timestep CE tensor instead of a scalar.
#
# 2. Modified `prefix_rollout_ce()` to accept:
#      - temporal_analysis (bool flag, default False)
#      - epoch (int) for logging
#      - frame_rate, window_sizes_sec, output_dir
#    When flag is False -> original scalar return (backward-compatible).
#    When flag is True  -> returns (scalar, per_step_ce, window_stats)
#    and appends one JSON-lines record to output_dir/{mode}_ce_temporal.jsonl
#
# 3. Added `compute_normal_ce_temporal()` -- mirrors prefix_rollout_ce
#    but for teacher-forced (normal) CE. Call it from your solver
#    alongside the regular CE loss when you want temporal breakdown.
#
# 4. All new behaviour is gated behind `temporal_analysis=True`.
#    Passing temporal_analysis=False (default) changes nothing vs
#    the original code.
#
# 5. output_dir resolves automatically from $AUDIOCRAFT_DORA_DIR/temporal_logs
